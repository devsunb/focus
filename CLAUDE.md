...
### 접근성 권한 필수 (시작 시 1회 체크)

데몬은 시작 시 접근성 권한을 확인하고, 없으면 시스템 설정 팝업을 띄운 뒤 exit 1로 종료합니다.
launchd plist는 `KeepAlive > SuccessfulExit: true`로 설정되어 exit 0일 때만 재시작하므로,
권한 없이 exit 1로 종료하면 재시작 루프에 빠지지 않습니다.
